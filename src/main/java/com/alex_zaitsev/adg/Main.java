...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        if (analyzer.run()) {
            ...
            if(null != filters.getPackagePrefix() && !filters.getPackagePrefix().isEmpty()){
            	new Writer(resultFile).write(analyzer.getDependenciesPackagePrefix(filters.getPackagePrefix()));
            }else if(filters.isShowPackageDependencies()) {
                  new Writer(resultFile).write(analyzer.getDependenciesPackages());
            }else { // Class-level dependencies
